...
@section('title' , 'TMS - Home')
@section('content')
<marquee direction="left" 
        behavior="loop" 
        bgcolor="orange">
        Mango Season is Here . Buy your Mango Here...
    </marquee>
{{-- Hero Start --}}
  <section class="hero bg-cover bg-center h-full" style="background-image: url('{{asset('img/cover.jpg')}}');">
    <div class="flex justify-center text-center h-full w-full">
      ...
      <h1 class="md:text-5xl text-3xl leading-10">website name or Mango</h1>
      <p class="md:text-2xl pb-8">Descripttion Here</p>
...
{{-- Text Start --}}
  <section class="flex flex-col text-center h-96 my-12  border-black bg-gray-100">
    <h2 class="text-3xl font-bold text-amber-900 pt-20 pb-10">Why Mango</h2>
    <div class=" leading-loose tracking-wider text-sm px-2">  
      <p>Description 1.</p>
      <p>Description 2 <b> </b> Description 3</p>
      <p>Description 5</p>
    </div>
  </section>
{{-- Text End --}}
        ...
        <div class=" h-full w-full">
          <div class="absolute bottom-24 left-1/2 -translate-x-1/2">
            <form action="{{route('filter', $category->id)}}" method="POST">
                ...
                <button  class="text-xl md:text-3xl bg-white border border-white px-16 py-3 md:px-20 md:py-4 hover:bg-black h-fullover:text-white hover:border-black transition-all duration-200" type="submit">{{$category->name}}</button>
              </form>
          </div>

<center>
          <a href="{{$category->name}}  "><img src="{{ url ('uploads/image/'.$category->image) }}" alt=""width="200" height="200" >
...
{{-- Products start --}}
  ...
  {{-- Section title --}}
    <div class="flex justify-between">
        <div class="ml-4 text">
    <h2 class="text-lg md:text-3xl mb-2">Start buying</h2>
    ...
    @foreach($products as $product)
      ...
      <div class="h-4/5">
        <a href="{{route('product' ,$product->slug )}}">
          <img src="{{ url ('uploads/p_image/'.$product->image) }}" alt="{{$product->name}}" width="250" height="250">
        </a>
      </div>
      ...
      <div class="space-y-2">
        ...
        <div class="">
          <p class="font-semibold">{{(int)$product->price}} INRs</p>
        </div>
        <div>
          <a href="{{route('product' ,$product->slug)}}" class="text-end bg-yellow-900  px-10 py-1 text-white hover:bg-white hover:text-black hover:border-2 hover:border-yellow-900 transition duration-100">Buy Now</a>
        </div>
      </div>
    </div>
    @endforeach
