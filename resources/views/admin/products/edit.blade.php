...
<form action="{{route('admin.products.update' , $product->id)}}" method="POST" enctype="multipart/form-data">
 ...
 <div class="mb-6">
  ...
  <input type="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-md rounded-lg focus:ring-black focus:border focus:border-black  block w-1/2 p-2.5" name="name" value="{{$product->name}}">
 </div>
 
 <div class="mb-6">
  <label for="description" class="block mb-2 text-lg text-gray-600 font-bold">Description</label>
  <textarea name="description" id="" cols="30" rows="5" class="bg-gray-50 border border-gray-300 text-gray-900 text-md rounded-lg focus:ring-gray-400  focus:border-gray-400  block w-1/2">{{$product->description}}</textarea>
 </div>


 <label class="block mb-2 text-lg  text-gray-600 font-bold" for="image">Upload image</label>
 <div class="my-4 w-44 h-44">
    <img src="{{ url ('uploads/p_image/'.$product->image) }}" alt="" class="w-full h-full object-cover">
 </div>
 ...
 <input type="number" name="price" min="0" max="10000" class="block w-1/2 p-2 text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-gray-400  focus:border-gray-400 " value="{{$product->price}}">
</div>
<div class="mb-6">
